// Nav2.test.tsx - VERSÃO COMPLETA COM TODOS OS TESTES RECOMENDADOS
// ============================================
...
describe('Nav Component', () => {
  ...
  describe('Edge Cases de Segurança', () => {
    ...
    it('deve aceitar returnUrl relativo válido', async () => {
      // Configurar URL relativa válida
      (window as any).location.search = '?returnUrl=/dashboard';
    ...
    it('deve lidar com múltiplos logouts simultâneos', async () => {
      render(<Nav />);
      ...
      await waitFor(() => {
        // Deve chamar signOut apenas uma vez
        expect(mockSupabaseClient.auth.signOut).toHaveBeenCalledTimes(1);
      });
    });
  ...
  describe('Limpeza de Recursos', () => {
    ...
    it('deve cancelar requests pendentes ao desmontar', () => {
      ...
      // Deve ter chamado abort para limpar recursos
  ...
  describe('Performance e Memory', () => {
    it('não deve ter memory leaks com timers', async () => {
      ...
      // Avançar tempo e verificar se timers são limpos
      await act(async () => {
      ...
      // Não deve haver timers pendentes após unmount
