...
function New-HtmlReport {
    ...
    # CORREÇÃO: Construção dinâmica das linhas da tabela para evitar o erro de aninhamento de Here-String (`@`)

    # 1. Gerar as linhas da tabela
    $tableRows = $global:results | ForEach-Object {
        ...
        # Garante que o conteúdo seja escapado para HTML antes de ser inserido
        $detailsHtml = $_.Details | Out-String | ConvertTo-Html -Fragment
        $row += "<td>$detailsHtml</td>"
        $row += "</tr>"
        # Retorna a string da linha
        $row
    }
    
    # Juntar as linhas em uma única string com quebras de linha
    $tableRowsString = $tableRows -join "`n"

    # 2. Construir o conteúdo HTML principal usando um único here-string
    $htmlContent = @"
<!DOCTYPE html>
<html>
<head>
