// Nav1.test.tsx - VERSÃO CORRIGIDA
// ============================================
...
describe('Nav Component', () => {
  ...
  beforeEach(() => {
    ...
    // Mock do localStorage - FORMA SIMPLIFICADA
    ...
    // SOLUÇÃO PARA window.location - não mockar, usar approach diferente
    // O componente usará window.location.search, então vamos garantir que seja vazio
    // Deletando a propriedade location e recriando (apenas em ambiente de teste)
  ...
  afterEach(() => {
    ...
    // Restaurar location original se necessário
    if (originalLocation) {
  ...
  // ============================================
  // TESTES DE LOGOUT SIMPLIFICADOS
  // ============================================
  ...
  describe('Funcionalidade de Logout', () => {
    ...
    it('deve exibir estado de loading durante logout', async () => {
      ...
      // Não usar await para poder ver o estado de loading
      fireEvent.click(logoutButton);
    ...
    it('deve pedir confirmação se houver mudanças não salvas', async () => {
      ...
      // Simular mudanças não salvas clicando no link
      const createLink = screen.getByText('Create New Ticket');
      fireEvent.click(createLink);

      const logoutButton = screen.getByRole('button', { name: /log out/i });
      global.confirm = jest.fn(() => false); // Usuário cancela
  ...
  describe('Detecção de inatividade', () => {
    ...
    it('deve resetar timer ao detectar atividade do mouse', () => {
      ...
      // Deve ter resetado o timer (clearTimeout + setTimeout)
