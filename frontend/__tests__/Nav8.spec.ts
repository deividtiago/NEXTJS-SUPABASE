...
testWithMocks.describe('4. Segurança (100% Mockado)', () => {
  testWithMocks('deve prevenir redirect malicioso via returnUrl', async ({ page }) => {
    ...
    // CORREÇÃO: Navegar primeiro para uma página válida
    await page.waitForSelector('h1:has-text("Ticket List")');
    
    // CORREÇÃO: Usar abordagem mais simples para testar segurança
    // Tentar navegar para URL com parâmetro malicioso
    await page.evaluate(() => {
      window.dispatchEvent(new CustomEvent('navChange', { 
        detail: '/tickets?returnUrl=https://evil.com' 
      }));
    });
    
    // CORREÇÃO: Aguardar navegação e verificar que estamos na página correta
    await page.waitForSelector('h1:has-text("Ticket List")');
    
    // Verificar que a URL atual não contém o domínio malicioso
    ...
    // Verificar que não foi redirecionado para evil.com
